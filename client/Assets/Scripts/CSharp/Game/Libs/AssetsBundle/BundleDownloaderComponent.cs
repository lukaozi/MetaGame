	...
	public class BundleDownloaderComponent : Component
	{
		...
		public async ETTask StartAsync()
		{
			...
			try
			{
				...
				{
					versionUrl = GlobalConst.GlobalProto.GetUrl() + "StreamingAssets/" + "Version.txt";
				...
				while (true)
				{
					...
					while (true)
					{
						try
						{
							...
							{
								await this.webRequest.DownloadAsync(GlobalConst.GlobalProto.GetUrl() + "StreamingAssets/" + this.downloadingBundle);
